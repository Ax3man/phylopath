...
* Models that contain both binary and continuous variables now warn, once per
  ...
  artefact of the scales. Note that this concerns the coefficients only:
  comparison of the causal models is unaffected, because the d-separation tests are
  performed on unstandardized data, so `CICc`, the model weights and the ranking of
  models do not depend on the scale of the coefficients.
